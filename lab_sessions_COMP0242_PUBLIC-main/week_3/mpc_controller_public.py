...
def getSystemMatrices(sim, dyn_model, num_joints):
    """
    ...
    I = np.eye(num_joints)
    It = np.eye(num_joints)*time_step
    O = np.zeros([num_joints,num_joints])
    # TODO: Finish the system matric
    if np.any(dyn_model.conf['robot_pybullet']['motor_damping']):
        damp = dyn_model.conf['robot_pybullet']['motor_damping_coeff']
        damp = np.diag(damp)
        A = np.block([[I,It],[O,I-damp*time_step]])
    else:
        A = np.block([[I,It],[O,I]])
    B = np.block([[O],[It]])
...
def getCostMatrices(Q_val,R_val,num_joints):
    """
    ...
    Q = Q_val * np.eye(num_states)
    Q[num_joints:, num_joints:] = 0.0
    
    R = R_val * np.eye(num_controls)  # Control input cost matrix
    
    return Q, R


def main(Q_val = 1000,R_val = 0.1,show = False,savefig = True,savedata = True):
    # Configuration
    ...
    # Define the matrices
    A, B = getSystemMatrices(sim, dyn_model, num_joints)
    Q, R = getCostMatrices(Q_val,R_val,num_joints)
    ...
    for i in range(num_joints):
        ...
        if np.any(dyn_model.conf['robot_pybullet']['motor_damping']):
            damping = "_Damping"
        figname = f'Joint {i+1} with Q:{Q_val},R:{R_val}'+damping
        if savefig: 
            plt.savefig(savepath+'/'+figname+'.png')
        if show:
            plt.show()
    
    if savedata:
        dataname = f'data with Q:{Q_val},R:{R_val}'+damping
        save_data([q_mes_all, qd_mes_all],savepath,dataname)

    
    
if __name__ == '__main__':
    
    main(10000,1,show=True,savefig=False,savedata=False)