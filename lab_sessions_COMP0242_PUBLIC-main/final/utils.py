...
def plot_error(save_path,Qcoeff_init,Rcoeff_init,N_mpc_init):
    ...
    ## calculate the average x,y,theta error for both cases
    print("Average error no EKF")
    print(np.mean(abs(error),axis=0))
    print("Average error EKF")
    print(np.mean(abs(EKF_error),axis=0))
    plt.figure()
    plt.plot(EKF_error[:,0], label='x error',color='red')
    plt.plot(EKF_error[:,1], label='y error',color='blue')
    plt.plot(EKF_error[:,2], label='theta error',color='green')
    plt.plot(error[:,0], label='x error no EKF',color='red',linestyle='dashed')
    plt.plot(error[:,1], label='y error no EKF',color='blue',linestyle='dashed')
    plt.plot(error[:,2], label='theta error no EKF',color='green',linestyle='dashed')
    plt.legend()
    plt.grid()
    plt.show()
